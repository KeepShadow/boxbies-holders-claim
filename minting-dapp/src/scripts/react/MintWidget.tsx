...
interface Props {
  ...
  claim(): Promise<void>; /* whitelistMintTokens(mintAmount: number): Promise<void>; */
}
        ...
        {this.canMint() ?
          <div className={`mint-widget ${this.props.loading ? 'animate-pulse saturate-0 pointer-events-none' : ''}`}>
            ...
            <div className="controls">
              ...
              <button className="primary" disabled={this.props.loading} onClick={() => this.mint()}>Claim</button>
            </div>
          </div>
          :
          <div className="cannot-mint">
            ...
            {!this.props.isPaused ? <>You are not included in the <strong>claim</strong>.</> : <>The contract is <strong>paused</strong>.</>}<br />
          </div>
        }
      </>
    );
